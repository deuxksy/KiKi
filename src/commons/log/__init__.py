...
def get_logger(logger, level=logging.DEBUG, path='../log', prefix='%Y/%m/%d', suffix='%H%M%S',
    ...
    folder = '{}/{}'.format(config and config.get('log', 'path') or path,
                            time.strftime(config and config.get('log', 'prefix') or prefix, now))
    ...
    fileHandler = logging.handlers.RotatingFileHandler(filename='{}/{}'.format(folder, filename), maxBytes=fileMaxByte,
                                                       backupCount=10,
                                                       encoding=config and config.get('log', 'encoding') or encoding)
